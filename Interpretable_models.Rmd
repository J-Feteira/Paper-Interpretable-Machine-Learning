...
The RuleFit algorithm is using boosting on decision trees and then take the decision rules created and use these rules to do LASSO models. (<https://cran.r-project.org/web/packages/xrf/readme/README.html>)

<https://arxiv.org/pdf/1707.07149.pdf>

<https://www.r-bloggers.com/2015/10/rulefit-when-disassembled-trees-meet-lasso/>
...
<https://github.com/marjoleinF/pre#example-a-rule-ensemble-for-predicting-ozone-levels>

```{r}
## package pre
library(pre)

data_regression_gear_outcome$fuel <- as.factor(data_regression_gear_outcome$fuel)
data_regression_gear_outcome$offerType <- as.factor(data_regression_gear_outcome$offerType)
data_regression_gear_outcome$year <- as.factor(data_regression_gear_outcome$year)

test <- data_regression_gear_outcome[
  data_regression_gear_outcome$fuel!="Hydrogen"& data_regression_gear_outcome$fuel!="Ethanol",
  ]
## Hydrogen has not enough values (only 1) and ethanol 2
...
RuleFit_model_pre <- pre(
  price ~ mileage+gear, data=test, ntrees=10
  )

stopCluster(cl)

summary(RuleFit_model_pre)

coef(RuleFit_model_pre)
RuleFit_model_pre$glmnet.fit
```
...
tuneGrid <- caret_pre_model$grid(
  x = data_regression_gear_outcome[,c(1,5)], y = data_regression_gear_outcome$price, maxdepth = 3L:5L, 
...
test <- train(
  ...
  method = caret_pre_model, trControl = trainControl(number = 1), tuneGrid = tuneGrid)

stopCluster(cl)

save(test, file="best_model_RuleFit.RData")
...
```{r}
RuleFit_model_pre <- pre(
  gear ~ mileage+fuel+hp+offerType+year, data=data_regression_gear_outcome, family = 'binomial', 
  ntrees = 100, maxdepth = 3, tree.unbiased = TRUE
  )
```
