...
- hp: horse power of the vehicule.
...
Hp: the highest horse power that a car has is 850 so there is nothing weird for the hp feature.
...
## ask too much memory for my computer/R
#corrplot(cramer.matrix(data_cars), type="upper", diag=FALSE, tl.col="black")
...
ggplot(data_cars, aes(x=make, y=price))+
  ...
  labs(x="Make",y="Price of the car")
```
...
ggplot(data_cars, aes(x=fuel))+
  ...
  labs(x="Fuel",y="Number of cars using this fuel")
```
...
ggplot(data_cars, aes(x=fuel, y=price))+
  ...
  labs(x="Gear",y="Price of the car")
```
...
ggplot(data_cars, aes(x=gear))+
  ...
  labs(x="Gear",y="Number of cars using this type of gear")
```
...
It looks like that automatic gear increase the price of the car.
...
ggplot(data_cars, aes(x=offerType))+
  ...
  labs(x="Type of offer",y="Number of cars")
```
...
ggplot(data_cars, aes(x=offerType, y=price))+
  ...
  labs(x="Type of offer",y="Price of the car")
```
...
ggplot(aes(x=hp, y=price), data=data_cars) + 
  geom_point() +
  labs(subtitle="Horse power according to the price of the car", x="Horse power", 
       y="Price in euros") +  
  theme_minimal()
```

It looks like that the higher the horse power, the higher the price.
...
ggplot(data_cars, aes(x=year, y=price))+
  ...
  labs(x="Year",y="Price of the car")
```
