...
```{r}
logit_regression <- glm(gear~mileage+fuel+hp+year+offerType, data=data_regression_gear_outcome,
                        family="binomial")
...
```{r}
logit_regression <- glm(gear~fuel, data=data_regression_gear_outcome, family="binomial")
summary(logit_regression)
#stargazer(logit_regression, type="latex")
```
...
This part is for OneR models. OneR is a decision rule based on a unique rule. For this part I am going to create a model using the German cars dataset and with outcome being the gear. Here, we will let R split the features hp, mileage and price by using the function bin.
...
```{r}
library(RWeka)
JRip(gear ~ mileage+hp+fuel+offerType+year, data = data_OneR)
```
...
https://rdrr.io/cran/sbrl/man/sbrl-package.html
